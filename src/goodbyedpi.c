...
#define FILTER_UNBLOCK_WEBRTC "outbound and udp and !impostor and !loopback and (udp.DstPort == 443 or (udp.DstPort >= 50000 and udp.DstPort <= 50004))"
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        if (WinDivertRecv(w_filter, packet, sizeof(packet), &packetLen, &addr)) {
            ...
            else if ((do_fragment_udp) && (packet_type == ipv4_udp_data || packet_type == ipv6_udp_data)) {
                if (addr.Outbound) {
                    uint16_t dst_port = ntohs(ppUdpHdr->DstPort);
                    if (dst_port == 443 || (dst_port >= 50000 && dst_port <=50004)) {
                        send_udp_fragment(w_filter, addr, packet, packetLen, packet_data,
                                        packet_dataLen, packet_v4, packet_v6,
